...
Checks whether detected anomalies may be explained by calendar effects:
Indian public holidays, day-of-week patterns, month-end, and quarter-end.

Calendar effects are treated as supporting context only. An anomaly is
only flagged as a likely calendar artifact when it directly overlaps a
configured holiday. Region E's seasonal_index is a business-seasonality
signal and is not treated as an Indian holiday effect.
"""
...
# Holidays covering the synthetic dataset period (January–March 2024).
...
def _day_of_week_effect(df: pd.DataFrame, kpi: str) -> Dict[str, Any]:
    """Check whether a KPI has a meaningful weekday/weekend pattern."""
    ...
    if len(temp) < 7:
        return {"has_dow_effect": False, "status": "INSUFFICIENT_DATA"}
    ...
    if len(weekday_values) == 0 or len(weekend_values) == 0:
        return {"has_dow_effect": False, "status": "INSUFFICIENT_WEEKEND_WEEKDAY_DATA"}
...
def _holiday_overlap(date_range: pd.DatetimeIndex) -> List[Dict[str, str]]:
    """Return configured Indian holidays that fall within the date range."""

    holidays = []
    for date in date_range:
        date_str = pd.Timestamp(date).strftime("%Y-%m-%d")
        if date_str in INDIAN_HOLIDAYS_2024:
            holidays.append({"date": date_str, "name": INDIAN_HOLIDAYS_2024[date_str]})
    return holidays
...
def _holiday_anomaly_overlap(
    ...
    """Return anomaly indices that fall on a configured holiday."""

    if not detected_anomaly_indices or not holidays or "date" not in df.columns:
        return []

    dates = pd.to_datetime(df["date"], errors="coerce")
    holiday_lookup = {item["date"]: item["name"] for item in holidays}
    overlaps = []
    ...
    for idx in detected_anomaly_indices:
        ...
        if date_str in holiday_lookup:
            overlaps.append({"index": int(idx), "date": date_str, "holiday": holiday_lookup[date_str]})

    return overlaps


def _month_end_effect(df: pd.DataFrame, kpi: str) -> Dict[str, Any]:
    """Check whether values in the final three calendar days of a month differ materially."""

    if kpi not in df.columns or "date" not in df.columns:
        return {"has_month_end_effect": False, "status": "MISSING_COLUMN"}
    ...
    if len(temp) < 10:
        return {"has_month_end_effect": False, "status": "INSUFFICIENT_DATA"}
...
def _quarter_end_effect(df: pd.DataFrame, kpi: str) -> Dict[str, Any]:
    """Check whether values in quarter-end periods differ from the rest."""

    if kpi not in df.columns or "date" not in df.columns:
        return {"has_quarter_end_effect": False, "status": "MISSING_COLUMN"}
    ...
    if len(temp) < 10:
        return {"has_quarter_end_effect": False, "status": "INSUFFICIENT_DATA"}

    temp["is_quarter_end"] = (
        temp["date"].dt.is_quarter_end
        | (temp["date"].dt.month.isin([3, 6, 9, 12]) & (temp["date"].dt.day >= 29))
    )
...
def check_calendar_effects(
    ...
    Only flags is_likely_calendar_artifact=True when an anomaly index
    directly overlaps a configured Indian holiday. DOW, month-end, and
    quarter-end effects are returned as context but do not trigger the flag.
    """
    ...
    if holidays:
        holiday_names = ", ".join(f"{h['name']} ({h['date']})" for h in holidays)
        calendar_findings.append({
            "type": "HOLIDAY_IN_RANGE",
            "finding": f"Configured Indian holiday(s) present: {holiday_names}",
            "impact": (
                "Calendar context only. A holiday should only be treated as an anomaly "
                "explanation if the detected anomaly overlaps it."
            ),
        })

    holiday_anomaly_overlaps = _holiday_anomaly_overlap(df, detected_anomaly_indices, holidays)
    for overlap in holiday_anomaly_overlaps:
        calendar_findings.append({
            "type": "HOLIDAY_ALIGNED_ANOMALY",
            "finding": f"Detected anomaly on {overlap['date']} overlaps {overlap['holiday']}.",
            "impact": "HIGH — calendar effect is a plausible explanation and should be checked before taking action.",
        })
    ...
    for kpi in material_kpis:
        ...
        if result.get("has_dow_effect"):
            calendar_findings.append({
                "type": "DAY_OF_WEEK",
                "kpi": kpi,
                "finding": f"{kpi}: weekday/weekend difference is {result['weekday_weekend_diff_pct']:.1f}%.",
                "impact": "Calendar pattern exists. Individual anomalies should be compared against the same day-of-week baseline.",
            })
    ...
    for kpi in material_kpis:
        ...
        if result.get("has_month_end_effect"):
            calendar_findings.append({
                "type": "MONTH_END",
                "kpi": kpi,
                "finding": f"{kpi}: month-end values differ from the rest of the month by {result['month_end_diff_pct']:.1f}%.",
                "impact": "Month-end behavior may contribute to an apparent anomaly.",
            })
    ...
    for kpi in material_kpis:
        ...
        if result.get("has_quarter_end_effect"):
            calendar_findings.append({
                "type": "QUARTER_END",
                "kpi": kpi,
                "finding": f"{kpi}: quarter-end values differ from the rest of the period by {result['quarter_end_diff_pct']:.1f}%.",
                "impact": "Quarter-end behavior may contribute to an apparent anomaly.",
            })
    ...
    if is_likely_calendar_artifact:
        recommendation = (
            "Detected anomaly overlaps an Indian holiday. Verify whether the signal "
            "persists after controlling for the holiday before taking action."
        )
    elif calendar_findings:
        recommendation = (
            "Calendar patterns are present in the observed data, but no detected anomaly "
            "was directly linked to a configured holiday. Compare anomalies against "
            "day-of-week, month-end, and quarter-end baselines before acting."
        )
    else:
        recommendation = (
            "No material calendar pattern was detected. The anomaly is unlikely to be "
            "explained by the checked calendar effects."
        )
